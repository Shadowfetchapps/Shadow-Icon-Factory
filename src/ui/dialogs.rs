...
pub fn show_error(parent: &impl IsA<gtk::Window>, err: &Error) {
    let dialog = adw::MessageDialog::new(Some(parent), Some("Icon Factory"), Some(&err.human_message()));
...
pub fn show_about(parent: &impl IsA<gtk::Window>) {
    adw::AboutWindow::builder()
        ...
        .copyright("© 2026 Shadow Icon Factory contributors")
        .build()
        .present();
}
...
pub fn confirm(parent: &impl IsA<gtk::Window>, title: &str, body: &str, on_yes: impl Fn() + 'static) {
    let dialog = adw::MessageDialog::new(Some(parent), Some(title), Some(body));
    dialog.add_response("cancel", "Cancel");
    dialog.add_response("ok", "Overwrite");
